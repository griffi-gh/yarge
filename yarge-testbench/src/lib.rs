macro_rules! define_test {
  ...
  ($name: tt, $path: literal, $callback: tt, $setup: tt) => {
    define_test!($name, $path, ($callback), ($setup), (|_: &mut Gameboy, res: Result<usize, ::yarge_core::YargeError>| -> bool {
      match res {
        Ok(_) => false,
        Err(
  ...
  ($name: tt, $path: literal, $callback: tt, $setup: tt, $predicate: tt) => {
    ...
    fn $name () {
      use ::yarge_core::{Gameboy, YargeError};
...
macro_rules! define_test_mooneye {
  ($name: tt, $path: literal) => {
    define_test!($name, $path, (|gb: &mut yarge_core::Gameboy| {
